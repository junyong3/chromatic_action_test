...
import { useMutationWrap } from '@src/queries/useMutation'
...
function CouponCode({ disabled = false }: InputDataProps) {
  ...
  const clickValidateCouponCode = () => {
    ...
    mutate(
      NetworkService.commerce.post(
        COMMERCE_COUPON_API_PATH.VALIDATE_COUPON_CODE,
  ...
  const setIsActive = (e: React.SyntheticEvent, checked: boolean) => {
    ...
    LoadingService.show()
    mutate(
      NetworkService.commerce.patch(
        COMMERCE_COUPON_API_PATH.SET_ACTIVE(id as string),
        { isActive: checked }
      ),
      {
        onSuccess: ({ data }: any) => {
