...
} from '@pages/Member/Detail&Update/Tab/Props'
...
import { useBankListCall } from '@pages/Member/Detail&Update/Tab/query/useBankListCall'
import { RefundAccountCall } from '@pages/Member/Detail&Update/Tab/query/useRefundAccountCall'
...
import { useRefundAccountStatus } from '@pages/Member/Detail&Update/Tab/hooks/useRefundAccountStatus'
...
function MemberRefundAccountTab({ memberId }: MemberTabProps) {
  ...
  const removeRefundAccount = () => {
    LoadingService.show()
    remove(
      NetworkService.commercePayment.patch(
        COMMERCE_PAYMENT_API_PATH.REMOVE_REFUND_ACCOUNT,
        { userId: memberId }
      ),
      {
        onSuccess: () => {
  ...
  const onSubmit = useCallback(() => {
    formContext.handleSubmit((success) => {
      ...
      save(
        NetworkService.commercePayment.patch(
          COMMERCE_PAYMENT_API_PATH.SAVE_REFUND_ACCOUNT,
          { userId: memberId, ...success }
        ),
        {
          onSuccess: () => {
            onSuccess('save')
