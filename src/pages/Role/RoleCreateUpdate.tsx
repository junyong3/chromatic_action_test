...
function RoleCreateUpdate() {
  ...
  const { isSuccess: isSuccessPrepare, data: detailPrepare } =
    useQueryWrap<PrepareRoleRes>(['prepare'], () =>
      NetworkService.iam.get(IAM_API_PATH.ROLE_PREPARE)
    )
  ...
  const onClickSave = () => {
    ...
    if (id) {
      update(
        NetworkService.iam.put(
          IAM_API_PATH.ROLE(id as string),
          createRoleParams
        ),
        {
          onSuccess: onSuccess,
      ...
      create(
        NetworkService.iam.post<CreateRoleReq>(
          IAM_API_PATH.ROLE_LIST,
          createRoleParams
        ),
        {
          onSuccess: onSuccess,
