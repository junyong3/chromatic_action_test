...
import NoticeDataLayer from '@src/pages/Notice/Detail&Update/NoticeDataLayer'
...
import useNoticeCreateUpdate from '@pages/Notice/query/useNoticeCreateUpdate'
...
function NoticeCreateUpdate() {
  ...
  const {
    ...
    ['detailNoticeCNU', id],
    () =>
      NetworkService.commerce.get(COMMERCE_NOTICE_API_PATH.NOTICE_DETAIL(id)),
    {
      enabled: !!id,
    }
  )
  ...
  const onClickSave = () => {
    ...
    if (pageType === 'create') {
      CNU(
        NetworkService.commerce.post<CreateNoticeReq>(
      ...
      CNU(
        NetworkService.commerce.patch<UpdateNoticeReq>(
