...
import { COMMERCE_COUPON_API_PATH as API } from '@api/path/Commerce/couponPath'
import { rest } from 'msw'
const ROOT_PATH = API_ROOT_PATH.COMMERCE

export const CouponMock = {
  CouponList: rest.get(`${ROOT_PATH}${API.COUPON_LIST}`, (req, res, ctx) => {
    return res(
      ctx.status(200),
      ctx.json({
        code: 'SUCCESS',
  ...
  VerifyCouponCode: rest.post(
    `${ROOT_PATH}${API.VALIDATE_COUPON_CODE}`,
    (req, res, ctx) =>
      res(
        ctx.status(200),
        ctx.json({
  ...
  CreateCoupon: rest.post(
    `${ROOT_PATH}${API.CREATE_COUPON}`,
    (req, res, ctx) => {
      return res(
        ctx.status(200),
        ctx.json({
  ...
  CouponDetail: rest.get(
    `${ROOT_PATH}${API.COUPON_DETAIL('1')}`,
    (req, res, ctx) => {
      return res(
        ctx.status(200),
        ctx.json({
  ...
  CategoryList: rest.get(
    `${ROOT_PATH}${API.CATEGORY_LIST}`,
    (req, res, ctx) => {
      return res(
        ctx.status(200),
        ctx.json({
