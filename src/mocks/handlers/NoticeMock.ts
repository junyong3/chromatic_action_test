...
import { COMMERCE_NOTICE_API_PATH as API } from '@api/path/Commerce/noticePath'
import { rest } from 'msw'
const ROOT_PATH = API_ROOT_PATH.COMMERCE

export const NoticeMock = {
  NoticeCreate: rest.post(
    `${ROOT_PATH}${API.CREATE_NOTICE}`,
    (req, res, ctx) => {
      return res(
        ctx.status(200),
        ctx.json({
  ...
  NoticeList: rest.get(`${ROOT_PATH}${API.NOTICE_LIST}`, (req, res, ctx) => {
