...
import { MaterialDto, materialListRes } from '@api/model/MDM/Goods/material'
import { GridColDef } from '@mui/x-data-grid'
import { useQueryWrap } from '@queries/useQuery'
import Instance from '@api/Instance'
import { MDM_GOODS_MATERIAL_API_PATH } from '@src/api/path/MDM/Goods/materialPath'
import { useMaterialStore } from '@stores/MDM/Goods/material.store'
import { MaterialQueryKey } from '@domain/MDM/pages/Goods/Material/Props'
...
function MaterialDataGrid() {
  const searchCondition = useMaterialStore((state) => state.searchCondition)
  const [rows, setRows] = useState<MaterialDto[]>([])

  const { data, isLoading, refetch } = useQueryWrap<materialListRes>(
    MaterialQueryKey.materialList,
    () =>
      Instance.get(MDM_GOODS_MATERIAL_API_PATH.MATERIAL_LIST, searchCondition)
  )

  const columns: GridColDef<MaterialDto>[] = [
    {
      field: 'materialCode',
      headerName: '자재코드',
      renderCell: (params) => {
        return (
          <Link
            href={`${To.MDMGoodsMaterial}/1`}
            data-sb-kind={'pages/MDM/Goods/Material/Detail'}
            target="_blank"
            color="primary"
            underline={'hover'}
          >
