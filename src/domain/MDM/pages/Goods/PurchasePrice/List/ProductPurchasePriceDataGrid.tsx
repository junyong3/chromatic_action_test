...
function ProductPurchasePriceDataGrid() {
  const type = GoodsType.Product

  const [searchCondition, setPurchasePriceId] = useProductPurchasePriceStore(
  ...
  const columns: GridColDef<PurchasePriceTS>[] = [
    ...
    { field: 'register', headerName: '등록자', flex: 1 },
    {
      ...
      renderCell: ({ id }) => {
        return (
          <Link
            href={`${To.MDMGoodsProductPurchasePrice}/${id}`}
            data-sb-kind={'pages/MDM/Goods/ProductPurchasePrice/Detail'}
            target="_blank"
            color="primary"
            underline={'hover'}
          >
...
export default ProductPurchasePriceDataGrid
